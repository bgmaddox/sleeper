...
| 4 | All-Time completeness & loading limbo | ✅ done — commit `e509b30`; eager startup load of all years, `OptimalScoresByYear` restored on cache load (efficiency badges live), boot poller re-arms on year switch/refresh, All-Time/Playoffs placeholders self-retry, callbacks read `_data` snapshots, H2H default pair renders on open; 3 regression tests added; no cache rebuild needed (restore reads existing pickles) |
...
| 6 | Front-end quick wins (CSS vars, mobile nav, D3 leaks) | ✅ done — commit `e663b8f`; undefined CSS vars fixed (`--grid`→`--border`, `--text`→`--text-main`, `--card-bg`→`--bg-card`, `#1C3C54`→`var(--bg-card)`); mobile bottom nav forced to a single horizontal row + team chips wrap on their own full-width line (verified at 390×844); `renderScoreRace` teardown + `_waitForEl` last-wins cancellation; ~150 dead CSS lines deleted (threshold filter, survivor badges, ctrl-teams trio, Bootstrap shim, `.btn-group`, legacy `.btn-refresh`, `.tab-selected`, `.playoff-icon--star`); Plotly template right-sized (height 1000→580, title 45→24, subtitle 30→15, margin t 130→60). Corrections to Phase 4: `pr-th-sorted-asc/desc` are NOT dead (`tablesort.js` emits them — kept), and the empty race/heatmap/draft D3 containers are lazy-by-design behind "▶ Load Chart" buttons, not a rendering bug |
| 7 | Config to data files + I/O hardening | ✅ done — commit `92f22e3`; `roster_ids`/`leagueNumbers_Dict`/`SURVIVOR_LEAGUE_IDS`/`SIDE_BET_SEASONS` extracted to `config/*.json` (in `config/`, not `data/` — `Data/` is gitignored and case-collides on macOS), loaded at import with int-key conversion, values verified byte-identical to the old literals; `roster_ids_2024`-is-`roster_ids_2023` shared-object alias fixed (JSON yields distinct dicts); `AVAILABLE_YEARS` now derived from league IDs so "add a season" is JSON-only; all 11 `data_loader` HTTP calls routed through `_get_json` (30s timeout + `raise_for_status`), 3 `sleeper_core` calls hardened likewise; week-loop broad `except` narrowed — network errors now abort the year load loudly instead of caching a partial season; 12 regression tests added (`tests/test_config.py`); no cache rebuild needed (config values unchanged) |
| 8 | Retire the notebook (Sleeper_v3 thin wrapper) | ✅ done — commit `f21246f`; `Sleeper_v2.ipynb` moved to `archive/` (PickEm cells 34, 49–52, 100–102, 141 preserved for Session 11); `Sleeper_v3.ipynb` created as a thin wrapper (15 cells: class overview + verified example calls into `data_loader`/`sleeper_core`, every example smoke-tested against 2025 cache); "authoritative notebook" claim replaced in project CLAUDE.md, parent `CODING/CLAUDE.md` (which also had stale `dashboard.py` run instructions — fixed), and `.claude/structure.md`; `app.py` SECTION MAP de-numbered to grep-able marker names (drift-proof) and missing "Clientside callbacks" section added; suite green 205 passing, no cache rebuild needed (no data-path changes) |
| 9 | Playoff Calculator semantics + presentation | ✅ done — commit `47a0cd5`; checkpoint semantics rebuilt: played `as_of_week` games now fold into standings (`_fold_played_results`) and later weeks are simulated as 50/50 *even when since played*, so retroactive checkpoints reconstruct genuine odds instead of collapsing to 0%/100% (verified 2024 W10: probs 3.8%–99.0%, sum = 6.0 spots, folded records exact); rematch-pair indexing guarded (positional prefix mapping replaces tuple-identity lookup in both enumerators); tie handling fixed in `Week` (0.5 wins each, guarded against single-team playoff-bye groups; `Won` stays int while history is tie-free) and in the calculator fold; `LINEUP_SLOTS` constant + `validate_lineup_slots()` warns on League bootstrap if Sleeper `roster_positions` drift from the hardcoded optimal-lineup slots; UI: card subtitle "After Week N", `.chart-footnote` with 50/50 + frozen-PF + retroactive caveats, trajectory axis/hover "After Week"; perf: clinch/elim numbers gated behind opt-in flag (nothing reads them; they cost a full enumeration per team) and `NUMPY_EXACT_BIT_LIMIT` 29→20 (week-10 checkpoint 19.5s→~1.5s via MC); `playoff_probs` cache key bumped to v2 + stale v1 pickles deleted; 14 regression tests added (`tests/test_playoff_calc.py`). Note: the "n games on the H2H win pie" item resolved by context — the pie lives in `Season.MatchupPreview_WinPercentage`, which is dormant *and* broken (calls `OppWinPercentageTable`, defined only on `AllTime`) and rendered nowhere; the live H2H tab already shows an "All-time matchups: n" stat card |
| 10 | Boilerplate consolidation | ✅ done — commit `4b0389d`; `_graph()`/`_err_graph()` helpers in `app.py` replace 42 copy-pasted `dcc.Graph(config=…, style=…)` calls and 15 error-graph copies (the one local `_err_graph` def deleted; survivor graphs keep their `id=` via kwargs); `TeamColorsMixin` in `sleeper_core.py` — `Week`/`Season`/`AllTime`/`SideBet` now inherit `SetTeamColors`/`UpdateColors`/`UpdateColors2` from one implementation (subclasses override only `_default_teamcolors`; ~130 duplicated lines gone; 5 mixin tests in `tests/test_teamcolors.py`); `d3charts.js` single-source theme — `_theme()` reads CSS `:root` vars via `getComputedStyle` (cached; `--success`/`--chart-grid` added to `style.css`), plus `_getContainer`/`_setupChart`/`_makeTooltip`/`_styleAxis`/`_albersProjection`/`_mapUnavailable` helpers; all 9 renderers converted, every raw hex removed from renderer bodies (file 1509→1427 lines despite +113 helper lines). Suite green 242 passing; verified live: all 9 D3 charts render on Season/All-Time/This Week tabs, tick labels themed (Courier, `#BDE2FF`), Survivor `id=` graphs intact, zero console errors |
| Pi deploy (Tier 3) | Deploy webapp to Raspberry Pi + public link | ✅ done 2026-07-19 — commit `ac5ed10` (root→`/legacy/` redirect); Pi repo fast-forwarded to main, fresh `.venv` with pinned deps, `.cache`/`Data`/`.env` rsynced (`URL_BASE_PATHNAME=/legacy/` restored on Pi), `sleeper.service` rewritten (gunicorn 1 worker/4 threads, `127.0.0.1:8503` behind existing Caddy `/legacy*` route), Tailscale Funnel enabled → public `https://rachett.tail504ae5.ts.net/legacy/`; verified end-to-end in browser (login + full This Week tab render, zero console errors beyond root favicon 404). Note: a pre-existing July-6 deployment of the May scaffold was found already running as `sleeper.service`/`deploy.sh sleeper` — superseded in place |
| 11 | Pick 'Em page (port game to webapp, Survivor-tab pattern) | ✅ done — commit `700a58b`; `PickEm` class ported from the archived notebook into `sleeper_core.py` (parses roster `points_by_leg`; `previous_picks` only retains the latest week on Sleeper's API, so per-pick detail is not reconstructible — all charts derive from weekly scores); 3 chart methods (score race, weekly-scores heatmap, season leaderboard w/ weeks-won ties split); `PICKEM_LEAGUE_IDS` added to `config/league_ids.json` (2025 only); `fetch_pickem_rosters/users` + `load_pickem_for_year` in `data_loader` (disk-cached, mirrors Survivor); new "Pick 'Em" tab in `webapp/app.py` (`_tab_pickem`, deep-link `?tab=pickem`, clipboard-check tab icon in CSS); 18 tests added (`tests/test_pickem.py`: config, synthetic-roster parse math incl. tie splits + empty/missing-metadata edges, cache-backed 2025 data checks, chart smoke); suite green 237 passing (was 219); verified live in browser: tab renders 3 charts, subtitle "2025 · 6 players · Season Complete · Leader: sgmaddox", zero console errors |
...
Archive `Sleeper_v2.ipynb`; create the planned **`Sleeper_v3.ipynb` as a thin wrapper** that imports `sleeper_core` + `data_loader` and calls chart methods — interactive chart generation without a 5,470-line logic copy; update both CLAUDE.md files ("authoritative notebook" claim); regenerate or de-number the `app.py` SECTION MAP (grep-able marker names only). **PickEm decision (made 2026-07-15):** still played — it will be ported to the webapp as its own page following the Survivor pattern, in a later session (tracked as Session 11). When archiving `Sleeper_v2.ipynb`, preserve the PickEm cells (do not discard) so the port has a reference.
